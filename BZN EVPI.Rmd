...
NMB =(𝜆×Effectiveness)−Cost

where λ is the willingness-to-pay (WTP) threshold per QALY.
...
set.seed(123)
  ...
  P_card<-invbeta(mu=0.0189, low=0.01, high=0.03, n=repeats)
  CaMorR<-generate_rr_lognormal(1.1,5,2.57, n=repeats)
  DiMorR<-generate_rr_lognormal(1,2, 1.25, n=repeats)
  P_dig<-invbeta(mu=0.035,low=0.01, high=0.04, n=repeats)
...
values<-c(EVwPI, EVwCI, EVPI)
names(values)<-c("EVwPI", "EVwCI", "EVPI")
values
```
The per year EVPI
```{r}
EVPI/20
```
The per per person per year EVPI
```{r}
EVPI/20/54754
```
```{r}
...
ggplot(psa_df, aes(x = 1:nrow(psa_df), y = cum_mean_NMB)) +
  ...
  labs(
    ...
    y = "Cumulative Mean Incremental NMB (€)"
  ) +
  theme_minimal()+ 
