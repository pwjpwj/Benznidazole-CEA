...
P_card<-P_card_v[which.min(reps_SMSE)]
CaMorR<-CaMorR_v[which.min(reps_SMSE)]
DiMorR<-DiMorR_v[which.min(reps_SMSE)] # Cucunubá et al. Parasites&Vectors 2016
P_card_t<-0.019*0.49
P_dig<-P_dig_v[which.min(reps_SMSE)]
P_dig_t<-0.011*0.88
...
ggplot(deaths_chagas_disease2, aes(x = year, y = count)) +
  geom_point(color="red") + 
  geom_point(aes(y=deaths), color="blue")+
  geom_smooth(aes(y = count, color = "Count"), method = "lm", se = FALSE) +
  geom_smooth(aes(y = deaths, color = "Deaths"), method = "lm", se = FALSE) +
  labs(
    title = "Evolution of Deaths Over Time",
    x = "Year",
